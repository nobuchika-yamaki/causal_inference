...
class Agent:
    ...
    def step(self, y: float, env_info: Dict[str, float]) -> Dict[str, Any]:
        ...
        if mode == "intact":
            ...
            # per-step random sign: breaks systematic transmission (behaviour -> chance)
            causal_direction_used = cd_true * (1.0 if self.shuffle_rng.random() < 0.5 else -1.0)
        elif mode == "zero":
            # d(t) contributes nothing; probes alternate. C(t) and all else intact.
            causal_direction_used = 0.0
        elif mode == "flip":
            # systematic sign inversion: predicts below-chance probing if d(t) drives action
            causal_direction_used = -cd_true
        elif mode == "time_shuffle":
            # a past d(t) at a random lag: correct magnitude/statistics, wrong temporal alignment
